...
class QiniuCheck
  class << self
    def media_process(media_path, kind)
      if kind == '1'
        image_process(media_path)
      elsif kind == '2'
        video_process(media_path)
      end
    end

    def image_process(image_path)
      rb = '{"#{image_path}":$(key),"width":$(imageInfo.width),"height":$(imageInfo.height)}'
      put_policy = Qiniu::Auth::PutPolicy.new('marry')
      put_policy.return_body = rb

      _token = Qiniu::Auth.generate_uptoken(put_policy)

      path = image_path.path

      _code, result, _response_headers = Qiniu::Storage.upload_with_put_policy(
        put_policy,
        path
      )
    ...
    def video_process(video_path)
      ...
      put_policy.persistent_ops = [
        'avthumb/mp4/vb/1000k/vcodec/libx264/acodec/libfaac/s/640x480;',
        'avthumb/mp4/vb/1500k/vcodec/libx264/acodec/libfaac/s/1024x768;',
        'avthumb/flv/ar/44100/vb/512k/vcodec/flv/acodec/libmp3lame/s/320x240;',
        "avthumb/m3u8/segtime/10/vb/1000k/vcodec/libx264/acodec/libfaac/s/640x480|saveas/#{file_name_base64_1};",
        "avthumb/m3u8/segtime/10/vb/1500k/vcodec/libx264/acodec/libfaac/s/1024x768|saveas/#{file_name_base64_2};",
        'vframe/jpg/offset/1/rotate/auto'
      ].join
      ...
      _code, result, _headers = Qiniu::Storage.upload_with_put_policy(put_policy, path)
    ...
    def watermark(url, mark_url, options = {})
      return if url.blank? || mark_url.blank?
    ...
    # 将一个图片的网址下载然后上传到七牛上
    def upload_url(url)
      rb = '{"image_path":$(key),"domain":"http://qnpic.hunliji.com/"}'
    ...
    def up_load_file(file)
      ...
      token = Qiniu.generate_upload_token(scope: 'qnpic',
                                          ...
                                          deadline: (Time.now + 30.seconds).to_i,
                                          expires_in: 12 * 60 * 60)
      result = self.upload_image_with_path(path, token)
      result['domain'] + result['image_path']
    end
