...
class QiniuController < ApplicationController
  def audio_up_token
    rb = '{"audio_path":$(key),"hash":$(hash)}'
    uptoken = Qiniu.generate_upload_token(scope: 'marry',
                                          return_body: rb,
  ...
  def image_up_token
    rb = '{"image_path":$(key),"hash":$(hash),"width":$(imageInfo.width),"height":$(imageInfo.height)}'
    uptoken = Qiniu.generate_upload_token(scope: 'fans-home1',
                                          return_body: rb,
                                          deadline: (Time.now + 30.seconds).to_i,
                                          expires_in: 12 * 60 * 60)
    render json: { uptoken: uptoken }
  ...
  def video_up_token
    return_body = '{"video_path":$(key),"hash":$(hash),"persistent_id":$(persistentId)})'
